CLASS lhc_ExcelHead DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.

    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR ExcelHead RESULT result.

    METHODS earlynumbering_create FOR NUMBERING
      IMPORTING entities FOR CREATE ExcelHead.

    METHODS uploadExcelData FOR MODIFY
      IMPORTING keys FOR ACTION ExcelHead~uploadExcelData RESULT result.

    METHODS FillFileStatus FOR DETERMINE ON MODIFY
      IMPORTING keys FOR ExcelHead~FillFileStatus.
...
CLASS lhc_ExcelHead IMPLEMENTATION.
  ...
  METHOD earlynumbering_create.
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<lfs_entities>).
      APPEND CORRESPONDING #( <lfs_entities> ) TO mapped-excelhead
  ...
  METHOD uploadExcelData.
    DATA: lt_rows         TYPE STANDARD TABLE OF string,
          ...
          lt_excel        TYPE STANDARD TABLE OF zlaw_cl_bp_xl_user=>gty_gr_xl,
          lt_data         TYPE TABLE FOR CREATE zlaw_i_xl_user\_ExcelData,
    ...
    READ ENTITIES OF zlaw_i_xl_user IN LOCAL MODE
    ENTITY ExcelHead
    ...
    IF ls_excel IS NOT INITIAL.
      DO lv_no_of_cols TIMES.
        ...
        IF lv_has_error = abap_true.
          APPEND VALUE #( %tky = lt_file_entity[ 1 ]-%tky ) TO failed-excelhead.
          APPEND VALUE #(
            %tky = lt_file_entity[ 1 ]-%tky
            %msg = new_message_with_text(
                     severity = if_abap_behv_message=>severity-error
                     text     = 'Wrong File Format!!' )
          ) TO reported-excelhead.
    ...
    READ ENTITIES OF zlaw_i_xl_user IN LOCAL MODE
    ENTITY ExcelHead BY \_ExcelData
    ALL FIELDS WITH CORRESPONDING #( keys )
    RESULT DATA(lt_existing_XLData).
    IF lt_existing_XLData IS NOT INITIAL.
      MODIFY ENTITIES OF zlaw_i_xl_user IN LOCAL MODE
    ...
    MODIFY ENTITIES OF zlaw_i_xl_user IN LOCAL MODE
    ENTITY ExcelHead CREATE BY \_ExcelData
    ...
    "Read Updated Entry
    READ ENTITIES OF zlaw_i_xl_user IN LOCAL MODE
    ENTITY ExcelHead ALL FIELDS WITH CORRESPONDING #( Keys )
  ...
  METHOD FillFileStatus.
    ...
    READ ENTITIES OF zlaw_i_xl_user IN LOCAL MODE
    ENTITY ExcelHead FIELDS ( EndUser FileStatus )
